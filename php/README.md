# PHP client for the Horse Racing API
...
foreach ($api->pages('racesSearch', ['region' => 'GB', 'from' => '2024-01-01']) as $r) {
    echo $r['race_date'], ' ', $r['course'], PHP_EOL;
}

// errors carry a stable code
try {
    $api->trainersStats('tr_3EDFD3A');
} catch (ApiException $e) {
    if ($e->code === 'upgrade_required') {
        echo "That one needs the Analyst plan.", PHP_EOL;
    }
}
```
